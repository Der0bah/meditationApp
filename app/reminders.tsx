...
export default function Reminders() {
  ...
  const days: (number | null)[] = Array.from({ length: startWeekday }, () => null).concat(
  ...
  const add = () => {
    if (!selected || !/^\d{2}:\d{2}$/.test(time)) {
      return Alert.alert("Missing info", "Select a date and enter time as HH:mm");
    }
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.bg }} contentContainerStyle={{ padding: 16 }}>
      ...
      />
      <Text style={{ color: theme.text, marginTop: 8, fontFamily: fonts.regular }}>
        Date: <Text style={{ color: theme.mutetext }}>{selected ?? "None"}</Text>
      </Text>
      <Text style={{ color: theme.text, marginBottom: 8, fontFamily: fonts.regular }}>
        Time: <Text style={{ color: theme.mutetext }}>{time || "None"}</Text>
      </Text>
      ...
      <Card>
        {reminders.length === 0 ? (
          <Text style={{ color: theme.mutetext, padding: 10, fontFamily: fonts.regular }}>
            No reminders yet.
          </Text>
        ) : (
          reminders.map((r) => (
            <View
              key={r.id}
              style={{ flexDirection: "row", alignItems: "center", gap: 8, padding: 10, borderTopWidth: 1, borderColor: theme.border }}
            >
              <Text>🕒</Text>
