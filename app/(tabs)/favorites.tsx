...
export default function Favorites() {
  ...
  return (
    <ScrollView style={{ flex: 1, backgroundColor: theme.bg }} contentContainerStyle={{ padding: 16 }}>
      ...
      {favMed.length === 0 ? (
        <Card>
          <Text style={{ color: theme.mutetext, padding: 12, fontFamily: fonts.regular }}>No favorites yet.</Text>
        </Card>
      ) : (
        <View style={{ gap: 12 }}>
          {favMed.map((m) => (
            <Card key={m.id}>
              <Pressable onPress={() => router.push(`/details/${m.id}`)}>
                <Image
                  source={{ uri: m.image }}
                  style={{ width: "100%", height: 160, borderTopLeftRadius: 18, borderTopRightRadius: 18 }}
                />
                <View style={{ padding: 12 }}>
                  <Text style={s.cardTitle}>{m.title}</Text>
                  <Text style={{ color: theme.mutetext, fontFamily: fonts.regular }}>
                    {m.tag} • {m.duration} minutes
                  </Text>
                </View>
              </Pressable>
            </Card>
